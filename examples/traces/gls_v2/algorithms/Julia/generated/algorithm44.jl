...
"""
    algorithm44(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
...
"""                    
function algorithm44(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
    # cost: 4.48e+08 FLOPs
    ...
    # tmp67: ml2, full, tmp12: ml0, full, tmp14: ml3, symmetric_lower_triangular
    ml4 = Array{Float64}(undef, 100)
    # (Z18 W19 Z18^T) = tmp14
    t1 = time_ns()
    ml4, ml3 = LAPACK.syev!('V', 'L', ml3)
    t2 = time_ns()
    println("$(run_id); $(t1); W, A = LAPACK.syev; 3343333.333333333; $(t2-t1)")

    # tmp67: ml2, full, tmp12: ml0, full, W19: ml4, diagonal_vector, Z18: ml3, full
    ...
    # W19: ml4, diagonal_vector, Z18: ml3, full, tmp95: ml6, full
    # tmp96 = (W19^-1 tmp95)
    t1 = time_ns()
    ml6 ./= ml4
    t2 = time_ns()
    println("$(run_id); $(t1); x ./= A; 100; $(t2-t1)")

    # Z18: ml3, full, tmp96: ml6, full
    ...
    t_end = time_ns()
    println("$(run_id); $(t_end); algorithm44; 447916766.6666666; $(t_end-t_start)")
    # tmp24: ml7, full
    # b = tmp24
    return (ml7)
end