...
"""
    algorithm8(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
...
"""                    
function algorithm8(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
    # cost: 4.56e+08 FLOPs
    ...
    # y: ml2, full, L2: ml1, lower_triangular, L15: ml4, lower_triangular, tmp143: ml3, full
    # tmp67 = (L2^-1 y)
    t1 = time_ns()
    trsv!('L', 'N', 'N', ml1, ml2)
    t2 = time_ns()
    ...
    # L2: ml1, lower_triangular, L15: ml4, lower_triangular, tmp143: ml3, full, tmp67: ml2, full
    # tmp70 = (L2^-T tmp67)
    t1 = time_ns()
    trsv!('L', 'T', 'N', ml1, ml2)
    t2 = time_ns()
    ...
    t_end = time_ns()
    println("$(run_id) $(t_end) algorithm8 455876667.6666666 $(t_end-t_start)")
    # tmp24: ml5, full
    # b = tmp24
    return (ml5)
end