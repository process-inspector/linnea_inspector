...
"""
    algorithm34(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
...
"""                    
function algorithm34(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
    # cost: 4.47e+08 FLOPs
    ...
    ml3 = qr!(ml3)
    t2 = time_ns()
    println("[#LT] $(run_id); $(t1); A = qr; 2666666.6666666665; $(t2-t1)")

    # y: ml2, full, L2: ml1, lower_triangular, tmp12: ml0, full, Q16: ml3, QRfact_Q, R17: ml3, QRfact_R
    # tmp67 = (L2^-1 y)
    t1 = time_ns()
    trsv!('L', 'N', 'N', ml1, ml2)
    t2 = time_ns()
    ...
    # tmp12: ml0, full, Q16: ml3, QRfact_Q, R17: ml3, QRfact_R, tmp67: ml2, full
    ...
    t_end = time_ns()
    println("[#LT] $(run_id); $(t_end); algorithm34; 447230000.0; $(t_end-t_start)")
    # tmp24: ml6, full
    # b = tmp24
    return (ml6)
end