using LinearAlgebra.BLAS
using LinearAlgebra

"""
    algorithm12(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})

Compute
b = ((X^T M^-1 X)^-1 X^T M^-1 y).

Requires at least Julia v1.0.

# Arguments
- `ml0::Array{Float64,2}`: Matrix X of size 1000 x 100 with property FullRank.
- `ml1::Array{Float64,2}`: Matrix M of size 1000 x 1000 with property SPD.
- `ml2::Array{Float64,1}`: Vector y of size 1000.
"""                    
function algorithm12(ml0::Array{Float64,2}, ml1::Array{Float64,2}, ml2::Array{Float64,1})
    # cost: 4.46e+08 FLOPs
    run_id = get(ENV, "LINNEA_RUN_ID", -1)
    t_start = time_ns()
    # X: ml0, full, M: ml1, full, y: ml2, full
    # (L7 L7^T) = M
    t1 = time_ns()
    LAPACK.potrf!('L', ml1)
    t2 = time_ns()
    println("[#LT] $(run_id); $(t1); LAPACK.potrf; 333333333.3333333; $(t2-t1)")

    # X: ml0, full, y: ml2, full, L7: ml1, lower_triangular
    # tmp67 = (L7^-1 y)
    t1 = time_ns()
    trsv!('L', 'N', 'N', ml1, ml2)
    t2 = time_ns()
    println("[#LT] $(run_id); $(t1); trsv; 1000000; $(t2-t1)")

    # X: ml0, full, L7: ml1, lower_triangular, tmp67: ml2, full
    # tmp70 = (L7^-T tmp67)
    t1 = time_ns()
    trsv!('L', 'T', 'N', ml1, ml2)
    t2 = time_ns()
    println("[#LT] $(run_id); $(t1); trsv; 1000000; $(t2-t1)")

    # X: ml0, full, L7: ml1, lower_triangular, tmp70: ml2, full
    ml3 = Array{Float64}(undef, 1000, 100)
    blascopy!(1000*100, ml0, 1, ml3, 1)
    # tmp12 = (L7^-1 X)
    t1 = time_ns()
    trsm!('L', 'L', 'N', 'N', 1.0, ml1, ml0)
    t2 = time_ns()
    println("[#LT] $(run_id); $(t1); trsm; 100000000; $(t2-t1)")

    # X: ml3, full, tmp70: ml2, full, tmp12: ml0, full
    ml4 = Array{Float64}(undef, 100)
    # tmp21 = (X^T tmp70)
    t1 = time_ns()
    gemv!('T', 1.0, ml3, ml2, 0.0, ml4)
    t2 = time_ns()
    println("[#LT] $(run_id); $(t1); gemv; 200000; $(t2-t1)")

    # tmp12: ml0, full, tmp21: ml4, full
    ml5 = Array{Float64}(undef, 100, 100)
    # tmp14 = (tmp12^T tmp12)
    t1 = time_ns()
    syrk!('L', 'T', 1.0, ml0, 0.0, ml5)
    t2 = time_ns()
    println("[#LT] $(run_id); $(t1); syrk; 10000000; $(t2-t1)")

    # tmp21: ml4, full, tmp14: ml5, symmetric_lower_triangular
    # (L15 L15^T) = tmp14
    t1 = time_ns()
    LAPACK.potrf!('L', ml5)
    t2 = time_ns()
    println("[#LT] $(run_id); $(t1); LAPACK.potrf; 333333.3333333333; $(t2-t1)")

    # tmp21: ml4, full, L15: ml5, lower_triangular
    # tmp23 = (L15^-1 tmp21)
    t1 = time_ns()
    trsv!('L', 'N', 'N', ml5, ml4)
    t2 = time_ns()
    println("[#LT] $(run_id); $(t1); trsv; 10000; $(t2-t1)")

    # L15: ml5, lower_triangular, tmp23: ml4, full
    # tmp24 = (L15^-T tmp23)
    t1 = time_ns()
    trsv!('L', 'T', 'N', ml5, ml4)
    t2 = time_ns()
    println("[#LT] $(run_id); $(t1); trsv; 10000; $(t2-t1)")

    t_end = time_ns()
    println("[#LT] $(run_id); $(t_end); algorithm12; 445886666.6666666; $(t_end-t_start)")
    # tmp24: ml4, full
    # b = tmp24
    return (ml4)
end